    ...
    for dec_device in "${DEC_DEVICE_IDS[@]}"; do
      for cvc_config in "${CONFIGS[@]}"; do
        ...
        CUDA_VISIBLE_DEVICES=$enc_cuda python -m vcmrs.encoder \
          ...
          --working_dir ./output/working_dir/$0/${cvc_config} \
          --output_bitstream_fname "bitstream/{bname}_qp${QP}_${cvc_config}_e${enc_device}_d${dec_device}.bin" \
          --output_recon_fname "recon/{bname}_qp${QP}_${cvc_config}_e${enc_device}_d${dec_device}" \
          --TemporalResample "Bypass" \
          --ROI "Bypass" \
          $fname

        encoded_bin=output/$0/bitstream/testv1_256x128_420p_qp${QP}_${cvc_config}_e${enc_device}_d${dec_device}.bin
        ...
        chk_rslt=$(check_md5sum ./output/$0/recon/testv1_256x128_420p_qp${QP}_${cvc_config}_e${enc_device}_d${dec_device}.yuv $decoded_yuv)
...
echo
python dbg_verify_crossplatform_checksum.py "`echo ${bin_md5[@]}`" "`echo ${yuv_md5[@]}`" "`echo ${QP_VALUES[@]}`" "`echo ${CONFIGS[@]}`" "`echo ${DEC_DEVICE_IDS[@]}`" "`echo ${ENC_DEVICE_IDS[@]}`"
