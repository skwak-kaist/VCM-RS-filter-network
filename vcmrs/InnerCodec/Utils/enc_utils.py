...
async def process_image(item, ctx):
  ...
  item.args.VCMBitStructOn = False
...
def get_cvc_encoder(ctx):
  return os.path.join(ctx.input_args.cvc_dir, 'bin', 'EncoderAppStatic')

def get_nnlf_model():
  cur_dir_path = os.path.dirname(os.path.abspath(__file__))
  return os.path.join(cur_dir_path, "..", "Models", "nnlf_lop2_model_int16.sadl")
...
async def encode_mixed_intra_inter(in_fname, out_bitstream_fname, out_recon_fname, chunk_info, cvc_log_fname, item, ctx):
  ...
  cvc_encoder = get_cvc_encoder(ctx)
  ...
  cmd.append(f'--NnlfModelName={get_nnlf_model()}')
  cmd.append(f'--VcmWorkingDir={item.args.working_dir}')
...
def unpad_frame(in_fname, out_fname, resolution, pad_h, pad_w, ffmpeg, item):
  ...
  makedirs(os.path.dirname(out_fname))
...
async def process_chunk_fallback(chunk_info, item, ctx):
  ...
  if item.args.InnerCodec == 'VTM':
    ...
    if item.args.NnlfSwitchEnable == 'Enabled':
      cmd.extend([f'--NnlfOption=1',])
      vcmrs.log("in enc_utils, NNLF is switched on slice level")
    else:
      vcmrs.log("in enc_utils, NNLF is turned off")
      cmd.extend([f'--NnlfOption=0',])
      
    if item.args.SpatialResample == 'AdaptiveDownsample' and item.args.SpatialResamplingFlag == 1:
      ...
      num_sc = len(adaptivedownsample_data.id_scale_factor_mapping)
      cmd.extend([ 
      ...
      for i in range(2,num_sc):
        cmd.extend([ 
              f'--ScalingRatioHor{i}={1/adaptivedownsample_data.id_scale_factor_mapping[i]:.2f}',
              f'--ScalingRatioVer{i}={1/adaptivedownsample_data.id_scale_factor_mapping[i]:.2f}',
              ])
      if item.args.SpatialResamplingFlag == 0:
        # Perform upscaling at VTM level
        cmd.extend(['--UpscaledOutput=0'])
      else:
        if not item._is_yuv_video or item._is_dir_video:
          cmd.extend(['--UpscaledOutput=2'])
        else:
          cmd.extend(['--UpscaledOutput=1'])
        #cmd.extend(['--UpscaledOutput=3'])
  else:
    cmd = [
      cvc_encoder, 
      '-c', cvc_encoder_cfg1,
...
# def get_frame_info_from_cvc_bitstream(bs_fname, ctx):
  ...
  for line in outs.splitlines():
    ...
    m = re.search('^POC\s+(\d+).*?\(\s+([A-Z_]+).*?QP(.*?)\).*?OUT_PIC_SIZE(\s+\d+),(\s+\d+).*?', line)
    if m:
       poc = int(m.group(1))
       #qp = int(m.group(2))
       qp = int(m.group(3))
       ...
       frame_info.qp = qp
       frame_info.sizes = [int(m.group(4)),int(m.group(5))]
...
# def set_extra_info_item(item, ctx, video_working_dir):
  ...
  frame_info_dicts = get_frame_info_from_cvc_bitstream(bs_fname, ctx, intra_yuv_fname).items()
  item.video_info.ic_dec_size_info = []
  for k1,v1 in frame_info_dicts:
    if k1 > len(item.video_info.ic_dec_size_info)-1:
      item.video_info.ic_dec_size_info += [None] * (k1 +1 - len(item.video_info.ic_dec_size_info))
    item.video_info.ic_dec_size_info[k1] = v1.sizes
  item.video_info.frame_qps = {k1:v1.qp for k1,v1 in frame_info_dicts}
