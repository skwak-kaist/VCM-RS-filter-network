...
def get_parameters_bitstream(item):
  ...
  if item.args.VCMBitStructOn:
    bs += struct.pack('H', item.args.FrameRate)
    bs += struct.pack('H', item.args.SpatialResample == "SimpleDownsample") # TODO: to be deleted?
...
def gen_vcm_bitstream(inner_bitstream_fname, item, out_bitstream_fname):
  ...
  if not item.args.VCMBitStructOn:
    ...
    ## use the VCM units structure.
    vcm_bs_mux = os.path.join(item.args.cvc_dir, 'bin', 'FrameMixerEncAppStatic')
    rsd_fname = item.rsd_fname
    if hasattr(item, "trd_enc_fname"):
      trd_fname = item.trd_enc_fname
    else:
      trd_fname = ""

    cmd = [vcm_bs_mux]
    cmd += ['-b', inner_bitstream_fname] # cvc output bitstream
    cmd += ['-o', out_bitstream_fname] # output
    cmd += ['-r', rsd_fname] # restoration data file name
    cmd += ['-t', trd_fname] # restoration data file name
    vcmrs.log('VCM bistream strucutre is packed.\n')

    vcmrs.debug(cmd)
    vcmrs.debug(f"File bin {inner_bitstream_fname} exist? {os.path.isfile(inner_bitstream_fname)}")
    err = utils.start_process_expect_returncode_0(cmd, wait=True)
    vcmrs.debug(f"File out {out_bitstream_fname} exist? {os.path.isfile(out_bitstream_fname)}")
    vcmrs.debug(f"File rsd {rsd_fname} exist? {os.path.isfile(rsd_fname)}")
    assert err==0, 'vcm bitstream structure failed'
...
def parse_vcm_bitstream(vcm_bitstream_fname, inner_bitstream_fname, item):
  ...
  with open(vcm_bitstream_fname, 'rb') as f:
    ...
    if not data==b'VCM':
      # temporal solution for image test in CI.
      return False
