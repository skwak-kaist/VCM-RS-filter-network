...
def process_file(item, ctx):
  ...
  item.isImg = bitstream_utils.parse_vcm_bitstream(item.fname, inner_bitstream_fname, item)
  ...
  makedirs(video_info.working_dir)

  video_info.ic_dec_size_info = []
...
def bitstream_demux(video_info, item, ctx):
  ...
  if item.args.VCMBitStructOn:
    ## first should unpack the VCM units. Then pack the potential merged LIC+VVC bitstream.
    vcmrs.log('VCM bistream strucutre is on at decode stage 1.\n')
    rsd_fname = get_rsd_fname(item)
    cvd_fname = get_cvd_fname(item)
    trd_fname = get_trd_fname(item)
    item.trd_dec_fname = trd_fname
    cmd = [ 
      bs_demux_app, 
      '-b', video_info.fname,
      '-r', rsd_fname, # restoration data file name
      '-v', cvd_fname, # coded video data file name
      '-t', trd_fname, # coded video data file name
    ]
    vcmrs.debug(cmd)
    err, outs = utils.start_process(cmd, wait=True, return_out=True)
    assert err==0, f"The unpack of VCM bitstream of file {video_info.fname} fails!"
    # collect the rsd data.
    with open(rsd_fname, 'rb') as f:
      if struct.unpack_from('H', f.read(2), 0)[0]:  # TODO: shouldn't it be [1] ?
        item.args.SpatialResample = 'SimpleDownsample' # TODO: to be deleted?           
      else:
        item.args.SpatialResample = 'AdaptiveDownsample'
...
def get_cvc_decoder(ctx):
  return os.path.join(ctx.input_args.cvc_dir, 'bin', 'DecoderAppStatic')
...
def cvc_decode(video_info, item, ctx, intra_fallback=False):
  ...
  #endif
  cvc_decoder = get_cvc_decoder(ctx)
...
def process_VTM_encoded(video_info, item, ctx):
  ...
  cvc_dec_recon_fname = get_cvc_dec_recon_fname(item)
  cvc_decoder = get_cvc_decoder(ctx)
  vcm_spatial_resampling_flag = get_spatial_flag_parameter(item)
...
def mix_intra_inter_yuv(cvc_dec_recon_fname, video_info, item):
  ...
  makedirs(os.path.dirname(item.inner_out_fname))
...
def get_trd_fname(item):
  makedirs(item.working_dir)
  return os.path.join(item.working_dir, f'TEMPAR_OUT.CFG')
  ...
  for line in outs.splitlines():
    if line.startswith('POC '):
      ...
      if idx > len(qps)-1:
        ...
        sizes += [None] * (idx +1 - len(sizes))
      sizes[idx] = [int(m.group(4)),int(m.group(5))]
      qps[idx] = qp

  video_info.num_frames = len(qps)
  video_info.frame_qps = qps
  video_info.intra_indices = intra_indices
  video_info.ic_dec_size_info = sizes
