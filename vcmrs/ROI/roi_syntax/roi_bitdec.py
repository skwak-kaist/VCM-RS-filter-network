...
def decode_roi_params(bytes_data, log = None):
  ...
  gops_rois = []  
  gops_rtg_resolutions = []  # possible different resolutions per each GOP
  for gop_idx in range(roi_gops_num):
    ...
    rtg_image_size = (rtg_image_size_x, rtg_image_size_y) # possible different resolutions per each GOP
    gops_rtg_resolutions.append( rtg_image_size )
    
  return roi_update_period, rtg_image_size_x, rtg_image_size_y, org_image_size_x, org_image_size_y, gops_rois, gops_rtg_resolutions # possible different resolutions per each GOP