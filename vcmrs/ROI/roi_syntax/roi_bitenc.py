...
def encode_gops_rois_params(roi_update_period, org_image_size_x, org_image_size_y, rtg_image_size_x, rtg_image_size_y, gops_rois, gops_resolutions, log): # possible different resolutions per each GOP
    ...
    for gop_idx in range(roi_gops_num):
      ...
      if gops_rois is not None:
        ...
        resolution = gops_resolutions[gop_idx] # possible different resolutions per each GOP
        if rtg_image_size_x is None or rtg_image_size_y is None:
          encode_syntax_unit_retargeting_data(result, org_image_size_x, org_image_size_y, resolution[0], resolution[1], rois, log)    
        else:
