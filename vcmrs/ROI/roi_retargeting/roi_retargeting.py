...
def find_ROIs(objects_for_frames, feather, desired_max_obj_size, width, height, start_idx, end_idx, visualize_prefix = None):
  ...
  for f in range(start_idx, end_idx+1):
    ...
    optimized_rois = roi_utils.optimize_objects(objects_for_frame_f)
  ...
  optimized_objects_for_all_frames = roi_utils.optimize_objects(objects_for_all_frames)
...
def include_background_ROI(rois, width, height, background_scale):
  
  output_rois = []
  for scale, roi in rois:
    if scale>=background_scale: continue
    output_rois.append( [ scale, roi ] ) 
  
  background = [background_scale, [0,0,width, height] ]
  output_rois.append(background)
  return output_rois
...
def _generate_grid_coords(rois, org_size_x, org_size_y):
  ...
  org_coords_x.sort()
  org_coords_y.sort()
  return rois, org_coords_x, org_coords_y

def _generate_grid_coords_even(rois, org_size_x, org_size_y):
  new_rois = []
  org_coords_x = [0, org_size_x]
  org_coords_y = [0, org_size_y]
  ...
  for roi in rois:                
    ...
    start_y = ((rect[1]+1)>>1)<<1
    end_y   = ((rect[3]+1)>>1)<<1

    new_rois.append([scale_factor, [start_x, start_y, end_x, end_y]])
  ...
  org_coords_x.sort()
  org_coords_y.sort()
  return new_rois, org_coords_x, org_coords_y
...
def _retarget_coords_even(org_coords, scale_factors):
  ...
  for b in range(bsize-1):    
    ...
    nom_org_roi_size = (org_roi_size>>1) * scale_factor_nom
    new_roi_size  = (nom_org_roi_size + scale_factor_den-1) // scale_factor_den
    new_roi_size = new_roi_size<<1
...
def _total_retarget_coords_even(org_coords, scale_factors, align_size, expected_size):
  new_coords, new_size = _retarget_coords_even(org_coords, scale_factors)
  ...
  for new_coord in new_coords:
    mod_coords.append((new_coord>>1)<<1)
  new_coords = mod_coords
  new_size = new_coords[-1]
  if expected_size is not None:
    if not new_size == expected_size:
      new_coords.append(expected_size)
      new_size = new_coords[-1]
  
  return new_coords, new_size

def generate_coords_even(org_size_x, org_size_y, rois, align_size_x, align_size_y, expected_rtg_size_x, expected_rtg_size_y):

  new_rois, org_coords_x, org_coords_y = _generate_grid_coords_even(rois, org_size_x, org_size_y)
  
  scale_factors_grid = _generate_scale_factors_grid(new_rois, org_coords_x, org_coords_y)
  ...
  rtg_coords_x,rtg_size_x = _total_retarget_coords_even(org_coords_x, x_wise_scale_factors, align_size_x, expected_rtg_size_x)    
  rtg_coords_y,rtg_size_y = _total_retarget_coords_even(org_coords_y, y_wise_scale_factors, align_size_y, expected_rtg_size_y)
...
def generate_coords(org_size_x, org_size_y, rois, align_size_x, align_size_y, expected_rtg_size_x, expected_rtg_size_y):
  ...
  _, org_coords_x, org_coords_y = _generate_grid_coords(rois, org_size_x, org_size_y)
...
def retarget_image(inp_image, bit_depth, inp_coords_x, inp_coords_y, out_coords_x, out_coords_y):  
  ...
  sum_max_num_pixels = inp_image_size_x*inp_image_size_y
  sum_bits = np.ceil(np.log2(sum_max_num_pixels)) + bit_depth
  
  sum_dtype = np.float64
  if sum_bits<=64: sum_dtype = np.uint64
  if sum_bits<=32: sum_dtype = np.uint32
  if sum_bits<=16: sum_dtype = np.uint16
  if sum_bits<=8:  sum_dtype = np.uint8
    
  v_sum = np.array([0,0,0], dtype=sum_dtype)
  v_num = 0  
  
  out_image = np.zeros( (out_image_size_y, out_image_size_x, 3), dtype=inp_image.dtype) 
  for by in range(bsize_y-1):        
    for bx in range(bsize_x-1):  
      ...
      if (inp_size_x==0) or (inp_size_y==0): 
        ...
        if num>0:          
          v_sum += np.sum( np.sum(inp_image[ inp_start_y1:inp_end_y1 , inp_start_x1:inp_end_x1 ], axis=0, dtype=sum_dtype), axis=0, dtype=sum_dtype )
          v_num += num
        continue
      ...
      roi_interpolation.BilinearInterpolationNumbaInt(out_image[ out_start_y:out_end_y , out_start_x:out_end_x ], inp_image[ inp_start_y:inp_end_y , inp_start_x:inp_end_x ] )
      #roi_interpolation.BilinearInterpolationCV2(out_image[ out_start_y:out_end_y , out_start_x:out_end_x ], inp_image[ inp_start_y:inp_end_y , inp_start_x:inp_end_x ] )
...
def retarget_image_onecomp(inp_image, bit_depth, inp_coords_x, inp_coords_y, out_coords_x, out_coords_y):  
  ...
  out_image = np.zeros( (out_image_size_y, out_image_size_x), dtype=inp_image.dtype) 
  for by in range(bsize_y-1):        
    for bx in range(bsize_x-1):  
      inp_start_x = inp_coords_x[bx]
      inp_start_y = inp_coords_y[by]
      ...
      if (inp_size_x==0) or (inp_size_y==0): 
        ...
        num = (inp_end_x1-inp_start_x1) * (inp_end_y1-inp_start_y1)
        if num>0:          
          out_image[ out_start_y:out_end_y , out_start_x:out_end_x ] = np.sum( np.sum(inp_image[ inp_start_y1:inp_end_y1 , inp_start_x1:inp_end_x1 ], axis=0, dtype=sum_dtype), axis=0, dtype=sum_dtype )//num
        else:
          out_image[ out_start_y:out_end_y , out_start_x:out_end_x ] = (1<<(bit_depth-1))
        continue
      ...
      roi_interpolation.BilinearInterpolationNumbaIntOneComp(out_image[ out_start_y:out_end_y , out_start_x:out_end_x ], inp_image[ inp_start_y:inp_end_y , inp_start_x:inp_end_x ] )
      #roi_interpolation.BilinearInterpolationNumbaInt(out_image[ out_start_y:out_end_y , out_start_x:out_end_x ], inp_image[ inp_start_y:inp_end_y , inp_start_x:inp_end_x ] )
      #roi_interpolation.BilinearInterpolationCV2(out_image[ out_start_y:out_end_y , out_start_x:out_end_x ], inp_image[ inp_start_y:inp_end_y , inp_start_x:inp_end_x ] )
