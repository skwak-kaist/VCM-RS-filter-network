...
def filter_sep_2D(image, hx, hy, border, implementation, min_value, max_value, ret_dtype):
  ...
  image_hx = np.zeros( (size_y       , size_x + HX-1, comps), dtype = image.dtype )
...
def filter_image_lowpass(image, min_value, max_value, fcx, fcy, N, filter_method, border, implementation):  #'lanczos',  'replicate'
  ...
  return filter_sep_2D(image, hx, hy, border, implementation, min_value, max_value, image.dtype)
...
def inpaint(img, mask, radius, flags): #emulation  for lack of OpenCV support for 3-component 16-bit inpainting
  out = None
  if img.dtype==np.uint8:
    out = cv2.inpaint(img,mask,radius,flags)
  elif len(img.shape)==2:
    out = cv2.inpaint(img,mask,radius,flags)
  else:
    out = np.zeros( img.shape, dtype=img.dtype)
    for c in range(img.shape[2]):
      out[:,:,c] = cv2.inpaint(img[:,:,c],mask,radius,flags)
  
  return out

def blur_background_scaled(ker, any_object, org_image, bit_depth, fc, scale):
  ...
  resized_inpainted_image = inpaint(resized_org_image,resized_any_object,5,cv2.INPAINT_NS)
  
  resized_blurred_image = filter_image_lowpass(resized_inpainted_image, 0, (1<<bit_depth)-1, fc, fc, ker, 'lanczos', 'replicate', np.int64)
...
def blur_background_scaled_no_aliasing(ker, any_object, org_image, bit_depth, fc, scale):
  org_width = org_image.shape[1] 
  org_height = org_image.shape[0] 
  ...
  resized_any_object = cv2.resize(any_object, dim, interpolation = cv2.INTER_AREA)
  resized_org_image  = cv2.resize(org_image,  dim, interpolation = cv2.INTER_AREA)
  
  resized_inpainted_image = inpaint(resized_org_image,resized_any_object,5,cv2.INPAINT_NS)
  
  inpainted_image = cv2.resize(resized_inpainted_image, (org_width, org_height), interpolation = cv2.INTER_LINEAR)
  
  mask = (any_object>0).astype(np.uint8)
  mask1 = 1-mask
  recomposed_image = inpainted_image*mask[:,:,None] + org_image*mask1[:,:,None]
    
  if ker>0:
    blurred_image = filter_image_lowpass(recomposed_image, 0, (1<<bit_depth)-1, fc, fc, ker, 'lanczos', 'replicate', np.int64)
  else:
    s = np.sum(recomposed_image,(0,1))  # DC only
    num_pixels = org_width * org_height
    color = (s + (num_pixels//2)) // num_pixels # rounding
    color = color.astype(org_image.dtype)
    blurred_image = np.full( (org_height, org_width,3), color, dtype=org_image.dtype)
...
def mix_images(img0, mask1, slope_size_half, img1, out_img = None):
  ...
  process_dtype = None
  if io_dtype == np.uint8:  process_dtype = np.uint16
  if io_dtype == np.uint16: process_dtype = np.uint32
  if io_dtype == np.uint32: process_dtype = np.uint64
  ...
  if out_img is None:
    out_img = np.zeros( (img0.shape[0], img0.shape[1], img0.shape[2]),dtype=io_dtype)
    
  for c in range(img0.shape[2]):
    p0 = np.multiply( img0[:,:,c], alpha0, dtype = process_dtype)
    p1 = np.multiply( img1[:,:,c], alpha1, dtype = process_dtype)
    out_img[:,:,c] = (p0 + p1)>>7
...
def save_descriptor_to_file(descriptor_file, content):
  with open(descriptor_file, 'w', newline="\n") as file:
    ...
    for frame in content.keys():
      file.write(f" {frame}:[ \n")
      for obj in content[frame]:
        objr = str(obj).replace(" ","")
        file.write(f"  {objr},\n")
      file.write(f" ],\n")
    file.write("}\n")
...
def optimize_objects(i_rois):
  ...
  for i_rect in i_rois:
    ...
    if not i_rect is None:
      ...
      if (i_rect[2]-i_rect[0])>0 or (i_rect[3]-i_rect[1])>0:
...
def extend_objects(objects_for_frames, feather, width, height):
  output_objects_for_frames = []
  for f in range(len(objects_for_frames)):        
    output_objects = []
    for i in range(len(objects_for_frames[f])):
      rect = objects_for_frames[f][i]
      #rect = rect_extend(rect, feather)
      #rect = rect_limit(rect, width, height)
      rect = rect_extend_limited(rect, feather, width, height)
      output_objects.append(rect)
    output_objects_for_frames.append(output_objects)
  return output_objects_for_frames

def extend_rois_and_improve_scale(rois, feather, width, height, scale_index_multiplier):
  output_rois = []
  for scale, rect in rois:
    nrect = rect_extend_limited(rect, feather, width, height)
    nscale = int(scale * scale_index_multiplier)
    output_rois.append( [nscale, nrect ] )
  return output_rois
...
def determine_roi_fallback(objects_for_frames, path, width, height, roi_size_limit, limit_min, limit_max, trd_enc_fname, TemporalResamplingPostHintFlag, PostHintSelectThreshold):
  vcmrs.log(f"RoI ratio limit: {limit_min} to {limit_max}")
  vcmrs.log(f"RoI size  limit: {roi_size_limit}")

  file_name=f"{path}/nnlf/nnlf_slice_option.txt"
  makedirs(os.path.dirname(file_name))

  if TemporalResamplingPostHintFlag:
    roi_post_hint_limit = PostHintSelectThreshold/100. 
    trph_current_frame_quality_valid_flag_list = []
  
  with open(file_name, 'w') as file:
    frame=0
    for bbs in objects_for_frames:
        template=np.zeros((height,width))
        for bb in bbs:
            x1, y1, x2, y2 = bb
            template[y1:y2, x1:x2] = 1
        roi_size=template.sum()
        roi_ratio=roi_size/template.size
        NnlfOption = 1 if roi_ratio > limit_min and roi_ratio <= limit_max and roi_size > roi_size_limit else 0

        if TemporalResamplingPostHintFlag:
          trph_current_frame_quality_valid_flag_list.append(1) if roi_ratio > roi_post_hint_limit else trph_current_frame_quality_valid_flag_list.append(0)

        file.write(f"{NnlfOption}\n")
        #vcmrs.log(f"Frame {frame}, roi_size={roi_size}, roi_ratio={roi_ratio}, NnlfOption={NnlfOption}")
        frame+=1

  if TemporalResamplingPostHintFlag:
    with open(trd_enc_fname, 'a') as file: 
      flag_line = f"trph_quality_valid_flag:{','.join(str(int(flag)) for flag in trph_current_frame_quality_valid_flag_list)}\n"
      file.write(flag_line)