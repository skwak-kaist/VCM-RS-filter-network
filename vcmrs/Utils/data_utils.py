...
def get_image_file_list(path):
  if path is None: return [] # bug-fix for empty list
...
def component_resize_420_to_444(input_component, output_component = None, bit_depth = None):
  height, width = input_component.shape
  dtype = input_component.dtype
  if output_component is None:
    output_component = np.zeros( (height*2, width*2), dtype = dtype)
  
  input_component32 = input_component
  if bit_depth:
    if dtype==np.uint16 and bit_depth>15:    
      input_component32 = input_component.astype(np.uint32)
    elif dtype==np.uint8 and bit_depth>6:    
      input_component32 = input_component.astype(np.uint16)            
  else:
    input_component32 = input_component.astype(np.uint32)
  
  output_component[::2,::2] = input_component
  
  output_component[1:height*2-1:2,0:width*2-1:2] = (input_component32[0:height-1,:]+input_component32[1:height,:])>>1
  
  output_component[0:height*2-1:2,1:width*2-1:2] = (input_component32[:, 0:width-1]+input_component32[:, 1:width])>>1
  
  output_component[1:height*2-1:2,1:width*2-1:2] = (input_component32[0:height-1,0:width-1]+input_component32[1:height,0:width-1]+input_component32[0:height-1,1:width]+input_component32[1:height,1:width]+2)>>2
  
  output_component[-1,:] = output_component[-2,:]
  output_component[:,-1] = output_component[:,-2]
...
def component_resize_444_to_420(input_component, output_component = None, bit_depth = None):
  if output_component is None:
  
    H, W = input_component.shape[0:2]
    return input_component[:H//2*2:2,:W//2*2:2].copy()
  
  H, W = output_component.shape[0:2]  
  output_component[:,:] = input_component[:H*2:2,:W*2:2]
  return  output_component

###############################################

def component_resize_422_to_444(input_component, output_component = None, bit_depth = None):
  height, width = input_component.shape
  dtype = input_component.dtype
  
  if output_component is None:
    output_component = np.zeros( (height, width*2), dtype = dtype)
  ...
  output_component[:,::2] = input_component
  
  output_component[:,1:width*2-1:2] = (input_component32[:, 0:width-1]+input_component32[:, 1:width])>>1
  output_component[:,-1] = output_component[:,-2]
...
def component_resize_444_to_422(input_component, output_component = None, bit_depth = None):
  if output_component is None:
  
    H, W = input_component.shape[0:2]
    return input_component[:,:W//2*2:2].copy()
  
  H, W = output_component.shape[0:2]  
  output_component[:,:] = input_component[:,:W*2:2]
  return  output_component
  
###############################################

def component_resize_444_to_444(input_component, output_component = None, bit_depth = None):
  if output_component is None:
...
def convert_bit_depth(img, input_bit_depth, output_bit_depth):
  if output_bit_depth==input_bit_depth:
    return img 

  return (img.astype(np.uint32) * ( (1<<output_bit_depth)-1) + ((1<<input_bit_depth)-2) ) // ((1<<input_bit_depth)-1)
   
  #if output_bit_depth>input_bit_depth:
  #  return img.astype(np.uint32) << (output_bit_depth-input_bit_depth)    
  #else output_bit_depth==input_bit_depth:
  #  return img >> (input_bit_depth-output_bit_depth)    

###############################################

def convert_yuv_to_yuvpng(input_fname, width, height, input_chroma_format, input_bit_depth, output_fname_template, output_bit_depth, num_frames = None, frame_skip=0):
  if num_frames:
    if num_frames<=0:
      return 0
  ...
  output_dtype = np.uint16 if output_bit_depth > 8 else np.uint8
  ...
  while True:
    ...
    yuv = np.zeros( (height, width, 3), dtype = input_dtype)    
    component_resize_444_to_444(np.frombuffer(y_buffer, dtype=input_dtype).reshape((height, width)), yuv[:,:,0], input_bit_depth)
    component_resize_uv_func(np.frombuffer(u_buffer, dtype=input_dtype).reshape(uv_shape), yuv[:,:,1], input_bit_depth)
    component_resize_uv_func(np.frombuffer(v_buffer, dtype=input_dtype).reshape(uv_shape), yuv[:,:,2], input_bit_depth)
    
    output_fname = output_fname_template % out_frame_idx
    
    yuv = convert_bit_depth(yuv, input_bit_depth, output_bit_depth).astype(output_dtype)    
    cv2.imwrite(output_fname, yuv)
...
def convert_yuvpng_to_yuv(input_fname_dir, input_bit_depth, output_fname, output_chroma_format, output_bit_depth):
  png_list = get_image_file_list(input_fname_dir)
  output_file = open(output_fname, 'wb')
  
  output_bytes_per_pixel = 2 if output_bit_depth > 8 else 1
  output_dtype = np.uint16 if output_bit_depth > 8 else np.uint8
  
  for png_file in png_list:
    img = cv2.imread( os.path.join(input_fname_dir, png_file), cv2.IMREAD_UNCHANGED)
    if img is None: continue
    
    img = convert_bit_depth(img, input_bit_depth, output_bit_depth)
      
    y = img[:,:,0]
    u = img[:,:,1]
    v = img[:,:,2]

    if output_chroma_format=="420":
      u = component_resize_444_to_420(u, None, input_bit_depth)
      v = component_resize_444_to_420(v, None, input_bit_depth)
    elif output_chroma_format=="422":
      u = component_resize_444_to_422(u, None, input_bit_depth)
      v = component_resize_444_to_422(v, None, input_bit_depth)
      
    output_file.write(y.astype(output_dtype).tobytes())
    output_file.write(u.astype(output_dtype).tobytes())
    output_file.write(v.astype(output_dtype).tobytes())
