...
######################################################
# tests md5 checksums files from inside given checksums file
def test_checksums(checksums_file_name):
  checksums_file_path = os.path.dirname(checksums_file_name)
  with open(checksums_file_name) as f:
    lines = f.readlines()
  ...
  for line in lines:
    pos = line.find(" ")
    file_md5 = line[0:pos]
    file_name = line[pos:].strip().lstrip("*").strip()
    full_file_name = os.path.join(checksums_file_path, file_name)
    
    try:
      md5 = hashlib.md5(open(full_file_name, 'rb').read())
      calculated_md5 = str(md5.hexdigest())
    except:
      calculated_md5 = "????????????????????????????????"
    if file_md5!=calculated_md5:
      vcmrs.log(f"File: {file_name} checksum mismach: stored {file_md5} != {calculated_md5} current")
      mismatch = True
  return mismatch
